...
n_subsets = 10
subset_ratio = 0.5
ckpt_dir = "trak_models-tmp/"
warmup_steps = 4000

import numpy as np
def sample_subset(seed: int, n: int, n_samples: int):
    np.random.seed(seed)
    choices = np.random.choice(n, n_samples, replace=False).tolist()
    return choices

valid_loader = get_dataloader(tokenized_ds['valid'], batch_size, False)
ds = TensorDataset(tokenized_ds['train']['input_ids'])

from torch.utils.data import Subset

os.makedirs(ckpt_dir, exist_ok=True)
for subset_idx in range(n_subsets):
    choices = sample_subset(42+subset_idx, len(ds), int(len(ds)*subset_ratio))
    subset = Subset(ds, choices)
    train_loader = DataLoader(subset, batch_size=batch_size, shuffle=True, collate_fn=collate_fn)
    ...
    model.cpu()
    torch.save(model.state_dict(), f"{ckpt_dir}/{subset_idx}.ckpt")

...
class GPTNeoXAttention(nn.Module):
    def __init__(self, config):
        ...
        if self.use_hook:
            ...
            self.query_key_value.register_forward_hook(forward_hook_fn)
            self.query_key_value.register_full_backward_hook(backward_hook_fn)
    ...
    def get_a_l_minus_1(self):
        ...
        return (
            torch.cat(
                [
                    self.inputs,
                    torch.ones((self.inputs.shape[0], self.inputs.shape[1], 1)).to(
                        self.inputs.device
                    ),
                ],
                dim=-1,
    ...
    def get_d_s_l(self):
        return self.d_s_l.detach().clone()

    def get_d_w_l(self):
        w_grad = self.query_key_value.weight.grad.detach()
        b_grad = self.query_key_value.bias.grad.detach().unsqueeze(-1)
        return torch.cat([w_grad, b_grad], dim=-1).detach().clone()
...
class GPTNeoXMLP(nn.Module):
    def __init__(self, config):
        ...
        if self.use_hook:
            ...
            def backward_hook_fn(module, grad_input, grad_output):
                self.d_s_l = grad_output[0]
            self.dense_h_to_4h.register_forward_hook(forward_hook_fn)
            self.dense_h_to_4h.register_full_backward_hook(backward_hook_fn)
    ...
    def get_d_w_l(self):
        w_grad = self.dense_h_to_4h.weight.grad.detach()
        b_grad = self.dense_h_to_4h.bias.grad.detach().unsqueeze(-1)
        return torch.cat([w_grad, b_grad], dim=-1).detach().clone()

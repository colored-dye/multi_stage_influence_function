...
if do_mlp:
    save_dir = "infls/cka-mlp/"
else:
    save_dir = "infls/cka-all/"


def feature_space_linear_cka(x, y):
    # x = x - torch.mean(x, 0, keepdim=True)
    # y = y - torch.mean(y, 0, keepdim=True)
    sim = torch.linalg.norm(x @ y.T)**2
    normx = torch.linalg.norm(x @ x.T)
    normy = torch.linalg.norm(y @ y.T)

    return sim / (normx*normy)
...
if __name__ == "__main__":
    ...
    for i, qi in enumerate(tqdm(choices_queries, desc="Influences")):
        ...
        for j, cj in enumerate(tqdm(choices_candidates[i], desc=f"Infl [{i}]", disable=True)):
            ...
            infl = feature_space_linear_cka(
                utils.ravel_named_params(query_grads)[0].unsqueeze(0),
                utils.ravel_named_params(cand_grads)[0].unsqueeze(0)
            )
            if torch.isnan(infl):
                infl = 0.0
            else:

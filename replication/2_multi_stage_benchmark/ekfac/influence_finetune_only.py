"""∇_θ L(z_test, θ_ft)^T H_θ_ft^(-1) ∇_θ L(z_pre, θ_ft)"""
...
device = "cuda:2"
model_path = "/storage/Model/bigscience/bloomz-560m"
factors_dir = "factors_finetune/"
DAMPING = 1e-4
topk = 100

results_dir = "../trace/bloom_only_finetune_infl_candidates/"
...
def main():
    ...
    ft_model = BloomForCausalLM.from_pretrained(model_path, device_map=device)

    logger.info("Loading EK-FAC factors...")
    factors = {"pt": {}, "ft": {}}
    # factors['pt']['QA'], factors['pt']['QG'], factors['pt']['Lambda'] = load_factors("../../ekfac-gptneox/factors/")
    factors['ft']['QA'], factors['ft']['QG'], factors['ft']['Lambda'] = load_factors(factors_dir)
    logger.info("EK-FAC factors OK.")
    ...
    for exp_i in range(n_experiments):
        ...
        with open(os.path.join(results_dir, f"{exp_i}.jsonl"), "w") as fp:
            for qi, test_sample in enumerate(tqdm(test_data_selections, desc=f"Exp {exp_i}")):
                ...
                query_ihvp = ekfac_utils.ekfac_ihvp(modules_of_interest=modules_of_interest,
                                                    QA=factors['ft']['QA'],
                                                    QG=factors['ft']['QG'],
                                                    Lambda=factors['ft']['Lambda'],
                                                    damping=DAMPING,
                                                    vec=query_grads)
                scores = []
                for ci in tqdm(reranker_candidates[exp_i][qi], desc=f"Query [{qi}]"):
                    candidate_text = train_data[ci]['text']
                    cand_grads = get_candidate_gradients(model=ft_model,
                                               modules_of_interest=modules_of_interest,
                                               tokenizer=tokenizer,
                                               text=candidate_text)
                    # cand_ihvp = ekfac_utils.ekfac_ihvp(modules_of_interest=modules_of_interest,
                    #                                    QA=factors['pt']['QA'],
                    #                                    QG=factors['pt']['QG'],
                    #                                    Lambda=factors['pt']['Lambda'],
                    #                                    damping=DAMPING,
                    #                                    vec=cand_grads
                    #                                    )

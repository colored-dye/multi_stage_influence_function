"""LAMA formatted with custom question format."""
...
lama_relations_path = "../data/LAMA/bloom_questions.jsonl"
...
def load_lama_relation(lama_base_dir):
    lama_files = [os.path.join(lama_base_dir, f) for f in os.listdir(lama_base_dir) if "json" in f]
...
def prepare_batch(texts: list[str], tokenizer: AutoTokenizer):
    tokenized_inputs = tokenizer(texts, padding='longest', return_tensors='pt')
    masked_indices = []
...
@torch.no_grad
def top_k_accuracy(model: BloomForCausalLM,
    ...
    for relation, samples in tqdm(lama_all.items()):
        ...
        for batch_i in tqdm(range(n_batches), desc=relation):
            batch = []
            targets = []
            for sample in samples[batch_i*BATCH_SIZE:(batch_i+1)*BATCH_SIZE]:
                prompt = parse_template(
                    template.strip(), sample["sub_label"].strip()
                ).strip()
                batch.append(prompt)
                target = " "+sample['obj_label'].lower()
            ...
            for i in range(len(batch)):
                ...
                decoded = [s.lower() for s in tokenizer.batch_decode(indices)]
                if targets[i] in decoded:
                    acc += 1
...
if __name__ == "__main__":
    lama = load_lama_relation(lama_base_dir)
    ...
    results_file = f"questions-{model_name}-topk={K}.json"
